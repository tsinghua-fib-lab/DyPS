...
class CityReal:
    ...
    def __init__(self, mapped_matrix_int, order_num_dist, idle_driver_dist_time, idle_driver_location_mat, order_time_dist, order_price_dist,
                 ...
                 global_flag="global", time_interval=10, fleet_rate=1, fleet_help=False,grid_neighbor=None, neighbor_dis = None, order_param = None):
        """
        ...
        self.grid_neighbor=grid_neighbor
        ...
        if self.order_param is not None:
            self.order_param_prob = np.zeros(self.order_param.shape, dtype=np.float32)
            order_num = np.sum(self.order_param, axis=1, keepdims=True)
            order_num[order_num==0]=1
            self.order_param_prob = self.order_param/ order_num

        self.record_real_order=0
        self.record_finish_order=0
        ...
        self.onoff_driver_location_mat = onoff_driver_location_mat      # no use
    ...
    def initial_order_random(self):
        """Initialize order distribution
        :param distribution: 'Poisson', 'Gaussian'
        :param dis_paras:     lambda,    mu, sigma
        """
        distribution_name = [self.distribution_name] * (self.M * self.N)
        distribution_param_dictionary = self.order_num_dist[self.city_time%self.n_intervals]
        distribution_param = [0] * (self.M * self.N)
        ...
        for idx, node in enumerate(self.nodes):
            if node is not None:
                node.order_distribution(
                    distribution_name[idx], distribution_param[idx])
        ...
        for _row in range(self.M):
            for _column in range(self.N):
                if self.mapped_matrix_int[_row][_column] != -100:
                    ...
                    for _layer in range(neighbor_max_layer):
                        _layer_neighbors[_layer] = []
                    for _layer in range(neighbor_max_layer):
                        for _node in get_layers_neighbors(i, j, neighbor_max_layer, self.M, self.N)[_layer]:
    ...
    def get_global_states(self):
        state= np.zeros((len(self.nodes),2))
        for i, node in enumerate(self.nodes):
            state[i,0]=node.idle_driver_num
            state[i,1]= max(node.order_num - node.fake_order_num - node.fleet_order_num,0)
        return state
    ...
    def get_order_list_PPO(self):
        ...
        for i, _node in enumerate(self.nodes):
            if _node is not None:
                ...
                if len(states) == 0:
                    continue
                states=[ states[i]+[feature[i][0]] for i in range(len(states))]
                order_state.append(states)
                order_idx.append(idx)
        return order_idx, order_state

    def get_order_info(self,node_id):
        # sequence决策，跨网格接单，输入网格id，输出state 和订单特征
        node= self.nodes[node_id]
        order_index, order_states, order_feature = [], [], []
        for i, _order in enumerate(node.orders):
            if not _order.served and _order._flag==0:
                order_states.append(_order.get_state()+[0])
                order_index.append((node._index, i))
                order_feature.append(_order.get_feature(self._global_entropy)+[0])
        for nei in node.neighbors:
            if nei is not None:
                for i, _order in enumerate(nei.orders):
                    if (not _order.served) and _order._flag==0  and _order.get_service_type()==-1:
                        order_states.append(_order.get_state()+[1])
                        order_index.append((nei._index, i))
                        order_feature.append(_order.get_feature(self._global_entropy)+[1])
    ...
    def get_dist_of_orders(self):
        ...
        for i, _node in enumerate(self.nodes):
            if _node is not None:
                ...
                n_orders[i][0] = _node.real_order_num
    ...
    def reset(self,mode='MDP'):
        ...
        self.step_generate_order_real(mode=mode)
        if self.fleet_help:
            self.step_generate_order_fleet_sample()
            #else:
        ...
        '''
        order_state: [begin node, end node, price, duration ,service type]    按照node存储
        order_feature : [end_entropy-start entropy , price , duration, service type]
        next_states_node : [node id, idle driver num , real order num , entropy , entropy-global, distribution]
        next_states : [node id, idle driver num , real order num, time, fleet driver num]
        global_orders_states : [order id ,begin id , end id, price , duration]  ，global的角度
        '''
        ...
        if mode=='PPO' or mode=='PPO2':
            order_idx, order_states = self.get_order_list_PPO()
            order_feature=None
        else:
            order_idx, order_states, order_feature = self.get_order_list()  # get order list of each grid
        # sample_order_idx, sample_order_list, sample_order_feature = self.get_sample_order_list()

        return states_node, states, order_states, order_idx, order_feature, global_orders_states
    ...
    def step_generate_order_real(self,mode='MDP'):
        # generate order at t + 1
        for node in self.nodes:
            if node is not None:
                # node_id = node.get_node_index()
                # generate orders start from each node
                #random_seed = node.get_node_index() + self.city_time+self.RANDOM_SEED
                random_seed = node.get_node_index() + self.city_time
                if self.order_param is None:
                    order_prob = None
                    neighbor_dis = None
                    generate = True
                else:
                    generate = False
                    order_prob = self.order_param_prob[node.get_node_index(),:,self.city_time%self.n_intervals]
                    neighbor_dis = self.neighbor_dis[node.get_node_index()]
                node.generate_order_real(self.l_max, self.order_time_dist, self.order_price_dist,
                                         self.city_time, self.nodes, random_seed,mode=mode,
                                         generate=generate, order_prob=order_prob, neighbor_dis=neighbor_dis)
    ...
    def step_generate_order_fleet_sample(self):
        # generate fleet order
        for node in self.nodes:
            if node is not None:
                node.generate_order_fleet_sample(self.city_time, self.nodes)
    ...
    def step_assign_order_broadcast_neighbor_reward_update(self, dispatch_action, neighbor_help=False,mode='MDP'):
        ...
        for node in self.nodes:
            all_real_num+= node.real_order_num
            all_new_num+= node.new_order
        self.record_real_order+=all_new_num

        for id,_orders in enumerate(dispatch_action):
            if self.neighbor_dispatch:
                node= self.nodes[id]
            else:
            ...
            #if mode=='PPO2' or self.fleet_help:
            node_rewards, all_order_num_node, finished_real_num_node, servers, fake_fin_node, fleet_fin_node, all_fake_node, all_fleet_node, idle_num_node, fleet_num_node = node.simple_order_assign_real_sample(_orders, self.city_time, self, self.fleet_help , self.neighbor_dispatch)
            #else:
                #node_rewards, all_order_num_node, finished_real_num_node, servers, fake_fin_node, fleet_fin_node, all_fake_node, all_fleet_node, idle_num_node, fleet_num_node = node.simple_order_assign_real(_orders, self.city_time, self, self.fleet_help , self.neighbor_dispatch)
            self.gmv += np.sum(node_rewards)
            #all_order_num += all_order_num_node
            finished_order_num += finished_real_num_node
            assert finished_real_num_node<=node.real_order_num
        ...
        self.record_finish_order+=finished_order_num

        #if all_order_num != 0:
        self.order_response_rate = self.record_finish_order / max(1., float(self.record_real_order))
        ...
        self.ori_order_num = all_real_num
    ...
    def step_set_distribution(self):
        order_dist = self.get_dist_of_orders()  # shape= (Nnode,1)
        # driver_dist = self.get_dist_of_idle_driver()
        driver_dist = self.get_dist_of_idle_n_driver()  # shape= (Nnode,1)
    ...
    def set_order_flag(self,id_pairs):
        for pair in id_pairs:
            node=self.nodes[pair[0]]
            node.orders[pair[1]]._flag=1
    ...
    def step(self, dispatch_actions, generate_order=1, mode='SAC'):
        ...
        self.step_set_entropy()     # global entropy 以及 每个网格相对global entropy的平均abs(偏差)

        # 1. Do dispatching and fleet management
        #self.check_nodewise()
        serve_driver_id, rewards = self.step_assign_order_broadcast_neighbor_reward_update(dispatch_actions,mode=mode)
        ...
        '''
        if mode=='SAC':
            order_idx, order_states, order_feature = self.get_order_list()
            next_states_node = self.step_get_node_observation()
            return next_states_node,order_states, order_idx, order_feature
            #next_states = self.step_get_observation()       # [node id, idle driver num , real order num, time, fleet driver num]
        # self.check_nodewise()
        if mode=='PPO2':
            order_idx, order_states = self.get_order_list_PPO()
            next_states_node = self.step_get_node_observation()
            return next_states_node,order_states, order_idx , None
        if mode=='PPO':
            order_idx, order_states = self.get_order_list_PPO()
            next_states_node = self.step_get_node_observation()
            return next_states_node,order_states, order_idx , None
        ...
        if mode=='MDP':
            ...
            next_states_node = self.step_get_node_observation()
            return  global_orders_states, next_states_node
        if mode=='SEQ':
            next_states_node = self.step_get_node_observation()
