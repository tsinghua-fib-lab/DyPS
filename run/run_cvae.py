...
def train(env, agent, writer=None, args=None, device='cpu'):
    best_gmv = 0
    best_orr = 0
    node_id = np.zeros((args.policy_num)).astype(int)
    s_gridid = np.arange(args.grid_num)
    z=np.zeros((16,args.policy_num))
    ...
    for iteration in np.arange(args.resume_iter, args.MAX_ITER):
        ...
        if iteration ==0 or iteration>upper_bound:
            ...
            cluster_prob, cluster = agent.action_hrl(s_gridid,z, device=device)
        ...
        for T in np.arange(args.TIME_LEN):
            ...
            if iteration<500000:
                for inter2 in range(len(state)):
                    id=cluster[inter2]
                    agent.cvae_l_buffer.append((np.array(id),state[inter2],action[inter2]))
            ...
            #### cvae_z
            id_all = torch.LongTensor(node_id).to(device)
            with torch.no_grad():
                z = agent.cvae_l.encode(id_all)
            z = z.cpu().data.numpy()  ##36,16
            z=z.T
        ...
        if iteration>upper_bound:
            if iteration!=0:
                agent.hrl_buffer.append((s_gridid,cluster_action,cluster_prob,np.sum(gmv)/100000.0,z))
        ...
        print(
            '>>> Time: [{0:<.4f}] Mean_ORR: [{1:<.4f}] GMV: [{2:<.4f}] Best_ORR: [{3:<.4f}] Best_GMV: [{4:<.4f}]'.format(
