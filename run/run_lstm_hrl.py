...
def train(env, agent, writer=None, args=None, device='cpu'):
    ...
    his=np.zeros((args.grid_num,200))
    trj_new=0
    trj_old=0
    ...
    for iteration in np.arange(args.resume_iter, args.MAX_ITER):
        ...
        if iteration >-1:
            ...
            cluster_prob, cluster = agent.action_hrl(s_gridid,his, device=device)
        ...
        for T in np.arange(args.TIME_LEN):
            ...
            if iteration<500000:
                ...
                state_l.append(state.unsqueeze(0))
                state_next_l.append(next_state.unsqueeze(0))
                action_l.append(action.unsqueeze(0))
                reward_l.append(reward.unsqueeze(0))
                ...
                if done or epoch_ended:
                    if iteration==0:
                        trj_old = torch.cat((torch.cat(state_l, 0), torch.cat(action_l, 0),
                                             torch.cat(reward_l, 0).unsqueeze(-1), torch.cat(state_next_l, 0)), -1)
                    his=agent.his(trj_old.to(device))
                    if iteration>0:
                        trj_new=torch.cat((torch.cat(state_l,0),torch.cat(action_l,0),torch.cat(reward_l,0).unsqueeze(-1),torch.cat(state_next_l,0)),-1)
                        agent.LSTM_buffer.append((trj_old,trj_new))
                        trj_old=trj_new
                    if done:
                        next_value = torch.zeros((agent.agent_num, 1))
                    elif epoch_ended:
        ...
        if args.parallel_queue == False:
            ...
            if iteration>upper_bound and  iteration < 500000:
        ...
        t_end = time.time()
        if iteration != 0:
            agent.hrl_buffer.append((s_gridid, cluster_action, cluster_prob, np.sum(gmv) / 100000.0,his))
        if iteration>upper_bound:
            if iteration > 5:
                agent.his_train(device)
                a=1
