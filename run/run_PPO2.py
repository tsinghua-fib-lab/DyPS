...
def get_parameter():
    ...
    args.driver_num=4000        # 改变初始化司机数量
...
def train(env, agent , writer=None,args=None,device='cpu'):
    ...
    for iteration in np.arange(args.resume_iter,args.MAX_ITER):
        ...
        for T in np.arange(args.TIME_LEN):
            ...
            #t0=time.time()
            MDP.cur_time=T
            action, value, logp, mask_agent, mask_order_multi, mask_action , next_state_rnn_actor, next_state_rnn_critic ,action_ids, selected_ids = agent.action(state,order,state_rnn_actor, state_rnn_critic,mask_order,order_idx ,device,sample=False,random_action=False, MDP=MDP,fleet_help=args.FM)
            ...
            if args.global_share:
                ...
                if args.full_share==False:
                    if args.fix_phi:
                        ...
                        reward= 1/np.sqrt(1+args.team_reward_factor**2)*reward+ args.team_reward_factor/np.sqrt(1+args.team_reward_factor**2)*team_reward
                else:
                    team_reward= torch.zeros_like(reward)
                    for i in range(args.grid_num):
                        num=1
            ...
            agent.buffer.push(state, next_state ,order, action, reward[:,None], value, logp , mask_order_multi, mask_action, mask_agent, state_rnn_actor.squeeze(0), state_rnn_critic.squeeze(0))
            ...
            if done or epoch_ended:
                if done:
                    ...
                    next_value, _ = agent.critic(next_state.to(device), agent.adj , next_state_rnn_critic.to(device)).detach().cpu()
...
def test(env, agent , test_iter=1 ,writer=None,args=None,device='cpu'):
    ...
    for iteration in np.arange(test_iter):
        ...
        for T in np.arange(args.TIME_LEN):
            ...
            MDP.cur_time=T
            action, value, logp, mask_agent, mask_order_multi, mask_action ,action_ids, full_prob, driver_num = agent.action(state,order,mask_order,order_idx ,device,sample=False,random_action=False, MDP=MDP, fleet_help=args.FM, need_full_prob=True, random_fleet=False and args.FM)
            ...
            if args.return_scale:
                ...
                reward/=args.reward_scale
            agent.buffer.push(state, order, action, reward[:,None], value, logp , mask_order_multi, mask_action, mask_agent)
        ...
        print('>>> Mean_ORR: [{0:<.4f}] GMV: [{1:<.4f}] Mean_KL: [{2:<.4f}] Mean_Entropy: [{3:<.4f}] Best_ORR: [{4:<.4f}] Best_GMV: [{5:<.4f}]'.format(
