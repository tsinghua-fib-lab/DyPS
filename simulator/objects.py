...
class Node(object):
    __slots__ = ('neighbors', '_index', 'orders', 'orders_samples', 'drivers',
                 'order_num', 'idle_driver_num', 'offline_driver_num','real_order_num',
    ...
    def get_info_time_distribution(self, l_max):    # 统计不同分布的duration数量
    ...
    def get_info_end_layer_distribution(self, layer_neighborhood, l_max):   #统计不同分布layer阶数的目的地数量
    ...
    def clean_order_fleet_fake(self):
        ...
        self.fake_order_num == 0
        self.fleet_order_num == 0
        assert self.real_order_num >= 0
        self.real_order_num= len(self.orders)
    ...
    def generate_order_fleet_sample(self,city_time, nodes, fleet_price=0):
        for ii in self.layers_neighbors_id[0]:
            self.orders.insert(0,Order(self, nodes[ii], city_time, 1, fleet_price, 0, service_type=1))
            self.fleet_order_num += 1
    ...
    def generate_order_real(self, l_max, order_time_dist, order_price_dist, city_time, nodes, seed,mode='MDP', generate=True, order_prob=None, neighbor_dis=None):
        """Generate new orders at each time step
        """
        # self.orders_samples_num = 0
        num_order_t = self.order_generator.sample(seed)
        #self.order_num += num_order_t
        # self.orders_samples_num += num_order_t
        self.new_order=num_order_t
        self.real_order_num += num_order_t
        self.fake_order_num = 0
        # self._last_order_list = copy(self.orders)

        # fake order for nodes
        self.orders.insert(0,Order(self, self, city_time, 1, 0, 0, service_type=0))
        self.fake_order_num += 1        # fake order for stay local
        ...
        if generate:
            ...
            if num_order_t>0:
                current_node_id = self.get_node_index()
                destination_node = np.random.choice(np.arange(len(order_prob)), num_order_t, p=order_prob, replace=True)
                for des in destination_node:
                    duration = neighbor_dis[des]
                    price_mean, price_std = order_price_dist[duration]
                    price = np.random.normal(price_mean, price_std, 1)[0]
                    price = price if price > 0 else price_mean
                    if duration==0:     # 在原地网格的接单
                        duration = 1
                    self.orders.append(Order(nodes[current_node_id],
                                        nodes[des],
                                        city_time,
    ...
    def remove_unfinished_order(self, city_time):
        ...
        for idx, o in enumerate(self.orders):
            ...
            if o.get_wait_time() + o.get_begin_time() < city_time:
                ...
                if o.get_service_type() == -1:
                    self.real_order_num-=1
            ...
            # order completed
            if o.get_assigned_time() + o.get_duration() + o.get_pickup_time()  == city_time and o.get_assigned_time() != -1:
                un_finished_order_index.append(idx)
                if o.get_service_type() == -1:
                    self.real_order_num-=1
                elif o.get_service_type() == 0:
    ...
    def simple_order_assign_real_sample(self, orders, city_time, city, fleet_help=False, neighbor_dispatch=False):
        rewards = []
        # num_assigned_order = min(self.order_num, self.idle_driver_num)
        served_orders = []
        served_fleet_orders = []
        ...
        ori_real_order_num = self.real_order_num
        ...
        for _order in orders:
            ...
            for key, assigned_driver in self.drivers.items():
                ...
                if not assigned_driver.onservice and assigned_driver.online and assigned_driver.get_service_type():
                    ...
                    # select fleet_order
                    _order=Order(self, _order.get_end_position(), _order.get_begin_time(), 1, _order.get_price(), 0, service_type=1)
        ...
        #assert len(orders)<=self.idle_driver_num , 'order num Error'
        ...
        for _order in rest_orders:
            ...
            for key, assigned_driver in self.drivers.items():
                ...
                if not assigned_driver.onservice and assigned_driver.online:
                    ...
                    if neighbor_dispatch==False:
                        assert _order.get_begin_position() is assigned_driver.node
                    else:
                        if _order.get_begin_position() is not assigned_driver.node:
                            if _order.get_wait_time() + _order.get_begin_time() - city_time <=0:
                                break
                            else:
                                _order.set_pickup_time(1)
                    real_finished+=1
                    _order.set_assigned_time(city_time)
                    #self.order_num -= 1
                    _order.get_begin_position().order_num-=1

                    rewards.append(_order.get_price())
                    served_orders.append(_order)
                    if _order.get_end_position() is not None:
                        if assigned_driver.get_service_type():  #  True for Joint Fleet Management and Order Dispatching, False for Order Dispatching
                            self.fleet_driver_num -= 1
                        assigned_driver.take_order(_order)
        ...
        if neighbor_dispatch:
            for node in self.neighbors:
                if node is not None:
                    node.orders=[i for j, i in enumerate(node.orders) if i not in served_orders]
        ...
        # ori_idle = self.idle_driver_num
        # ori_fleet = self.fleet_driver_num
        self.set_gmv(np.sum(rewards))

        return rewards, ori_real_order_num, real_finished, serve_driver_idx, fake_finished, fleet_finished, ori_fake_order_num, ori_fleet_order_num, ori_idle, ori_fleet

    def simple_order_assign_real(self, orders, city_time, city, fleet_help=False, neighbor_dispatch=False):
        rewards = []
        # num_assigned_order = min(self.order_num, self.idle_driver_num)
        served_orders = []
        served_fleet_orders = []
        ...
        for _order in rest_orders:
            ...
            for key, assigned_driver in self.drivers.items():
                if self.idle_driver_num <= 0:
                    break
                if _order.get_service_type() > 0:  # # 1+ for fleet management and represents neighbor node, 0 for staying, -1 for order dispatching
                    break
                if not assigned_driver.onservice and assigned_driver.online:
        ...
        #real_finished = ori_order_num - ori_fake_order_num - ori_fleet_order_num - self.order_num + self.fake_order_num + self.fleet_order_num
        real_finished= len(served_orders) - ori_fake_order_num+ self.fake_order_num
...
class Driver(object):
    ...
    def take_order(self, order):
        ...
        #if self.node is not order.get_begin_position():
            #print(self.node._index, order.get_begin_position()._index)
    ...
    def status_control_eachtime(self, city):
        ...
        if self.onservice is True:
            assert self.online is True
            order_end_time = self.order.get_assigned_time() + self.order.get_duration() + self.order.get_pickup_time()
...
class Order(object):
    __slots__ = ('_begin_p', '_end_p', '_begin_t',
                 '_t', '_p', '_waiting_time', '_assigned_time', '_served', '_service_type','_pickup_time', '_flag')
    ...
    def __init__(self, begin_position, end_position, begin_time, duration, price, wait_time=1, service_type=-1):
        ...
        self._pickup_time=0
        self._flag=0        # 0表示没人接， 1表示这个订单被接了 (用于sequence决策，时间没更新，但订单已经分配好了)
