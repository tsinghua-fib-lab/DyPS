...
class Actor(nn.Module):
    ...
    def forward(self, state, order, mask , adj=None ,hidden_state=None ,scale=True):
        # key embedding
...
class Critic(nn.Module):
    def __init__(self,grid_dim,time_dim,embedding_dim , state_contin_dim , order_contin_dim, init=True, use_rnn=False, use_GAT=False, use_neighbor_state=False , merge_method = 'cat' , use_auxi = False):
...
class PPO:
    ...
    def __init__(self,
        ...
        # optimizers
        self.actor = Actor(self.agent_num, self.TIME_LEN, 128, self.state_dim-2, self.order_dim-2,self.use_orthogonal, self.use_rnn, self.use_GAT and (not self.actor_decen), self.use_neighbor_state and (not self.actor_decen), self.merge_method , self.use_auxi or self.use_fake_auxi>0 ) 
        self.critic = Critic(self.agent_num, self.TIME_LEN, 128, self.state_dim-2, self.order_dim-2,self.use_orthogonal, self.use_rnn, self.use_GAT and (not self.critic_decen), self.use_neighbor_state and (not self.critic_decen), self.merge_method , self.use_auxi or self.use_fake_auxi>0)
        ...
        self.buffer= Replay_buffer(args.memory_size, self.state_dim, self.order_dim ,self.action_dim, self.hidden_dim,self.max_order_num, self.agent_num, self.gamma, self.lam,self.adv_normal, parallel_queue=self.parallel_queue, value_normalizer=self.value_normalizer, use_GAEreturn=self.use_GAEreturn ,actor_decen=self.actor_decen, critic_decen=self.critic_decen)
    ...
    def action(self, state,order, state_rnn_actor, state_rnn_critic ,mask,order_idx ,device='cpu', random_action=False ,sample=True, MDP=None, fleet_help=False,need_full_prob=False, random_fleet=False):
        ...
        if random_action:
            ...
            with torch.no_grad():
                probs, state_rnn_actor = self.actor(state, order.to(device), mask.to(device), self.adj ,state_rnn_actor.to(device))
                value, state_rnn_critic = self.critic(state, self.adj , state_rnn_critic.to(device))
        probs=probs.cpu()
        value=value.cpu()
    ...
    def update(self, device='cpu',writer=None):
        ...
        for iter in range(self.train_critic_iters):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(data_size)), batch_size, True):
                for _ in range(thread):
                    loss_critic, critic_info = self.compute_loss_critic(self.split_batch(index,data_critic))
    ...
    def compute_loss_actor(self,data):
        state, next_state, order, action, advantage, oldp, mask_order, mask_action, mask_agent, hidden_rnn_actor = data['state'], data['next_state'] ,data['order'], data['action'], data['advantage'], data['oldp'],data['mask_order'], data['mask_action'] ,data['mask_agent'], data['state_rnn']
        # Policy loss
...
class Replay_buffer():
    def __init__(self, capacity,state_dim, order_dim,action_dim, hidden_dim,max_order_num,agent_num , gamma=0.99, lam=0.95 , adv_normal=True, parallel_queue=False, value_normalizer=None, use_GAEreturn=False ,actor_decen=True, critic_decen=True ):
    ...
    def push(self, state, next_state ,order ,action, reward, value, p , mask_order , mask_action, mask_agent, state_rnn_actor, state_rnn_critic):
