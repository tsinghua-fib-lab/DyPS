...
class actor_hrl(nn.Module):
    def __init__(
        ...
        self.layer2 = nn.Linear(self.embedding_dim, 8)
...
class PPO:
    ...
    def __init__(self,
        ...
        self.hrl_actor_optimizer = torch.optim.Adam(self.hrl_actor.parameters(), lr=self.actor_lr)
        self.hrl_critic_optimizer = torch.optim.Adam(self.hrl_critic.parameters(), lr=self.critic_lr)
    ...
    def action_process(self, action):
        ...
        return action
    def action_hrl(self,id, device='cpu'):
        action_prob=self.hrl_actor(torch.LongTensor(id).to(device))
    ...
    def action(self, state, order, state_rnn_actor, state_rnn_critic, mask, order_idx, device='cpu',
               random_action=False, sample=True, MDP=None, fleet_help=False, need_full_prob=False, random_fleet=False,
               cluster_dict_=None, shunxu_=None):
        ...
        with torch.no_grad():
            for inter in range(policy_num):
                ...
                action_logit_grid, state_rnn_actor = self.actor_l[inter](s_grid, order_grid, mask_grid, self.adj,
                                                                         state_rnn_actor.to(device), return_logit=True)
    ...
    def ppo_hrl_train(self,device):
        ...
        if lenth < 1:
            ...
            state, action, action_prob, reward= [], [], [], []
            ...
            value_loss = F.smooth_l1_loss(
                self.hrl_critic(torch.LongTensor(state).to(device)), target_v)
            ...
            ### 更新actor
            advantage = (target_v - self.hrl_critic(torch.LongTensor(state).to(device))).detach()
            advantage = (advantage - torch.mean(advantage)) / torch.std(advantage)
            # 更新actor
            new_prob = self.hrl_actor(torch.LongTensor(state).to(device))
    ...
    def update(self, device='cpu', writer=None):
        ...
        for inter in range(policy_num):
            ...
            # 先集合别的agent数据
            other_agent_ids = [x for x in range(policy_num) if x != inter]
            data_other_actor_list=[]
            data_other_critic_list = []
            for inter1 in other_agent_ids:
                data_other = self.replay_buffer_l[inter1].get(device)
                data_size_other = data_other['state_actor'].shape[0]
                batch_num_other = int(np.round(data_size_other / batch_size / self.minibatch_num))
                if batch_num_other==0:
                    aaaa=1
                else:
                    data_actor_other = {
                        'state': data_other['state_actor'],
                        'next_state': data_other['next_state_actor'],
                        'order': data_other['order'],
                        'action': data_other['action'],
                        'advantage': data_other['advantage'],
                        'oldp': data_other['oldp'],
                        'mask_order': data_other['mask_order'],
                        'mask_action': data_other['mask_action'],
                        'mask_agent': data_other['mask_agent'],
                        'state_rnn':data_other['state_rnn_actor']
                    }
                    data_critic_other = {
                        'state': data_other['state_critic'],
                        'next_state': data_other['next_state_critic'],
                        'ret': data_other['ret'],
                        'value': data_other['value'],
                        'state_rnn': data_other['state_rnn_critic'],
                        'action': data_other['action'],
                        'oldp': data_other['oldp']
                    }
                    data_other_actor_list.append(data_actor_other)
                    data_other_critic_list.append(data_critic_other)
            data_other_actor_all=0
            data_other_critic_all=0
            keys_actor=data_actor_other.keys()
            keys_critic=data_critic_other.keys()
            for inter in range(len(data_other_actor_list)):
                if inter==0:
                    data_other_actor_all=data_other_actor_list[0]
                else:
                    for inter1 in keys_actor:
                        data_other_actor_all[inter1]=torch.cat([data_other_actor_all[inter1],data_other_actor_list[inter][inter1]],0)

            for inter in range(len(data_other_critic_list)):
                if inter==0:
                    data_other_critic_all=data_other_critic_list[0]
                else:
                    for inter1 in keys_critic:
                        data_other_critic_all[inter1]=torch.cat([data_other_critic_all[inter1],data_other_critic_list[inter][inter1]],0)




            data_size = data['state_actor'].shape[0]
            batch_num = int(np.round(data_size / batch_size / self.minibatch_num))
            if batch_num==0:
                #### 不加自己data
                aaaa=1
                data_size =data_other_critic_all['state'].shape[0]
                ...
                for iter in range(self.train_critic_iters):
                    ...
                    for index in BatchSampler(SubsetRandomSampler(range(data_size)), batch_size, True):
                        for _ in range(thread):
                            loss_critic, critic_info = self.compute_loss_critic_other(
                                self.split_batch(index, data_other_critic_all),self.split_batch(index, data_other_actor_all), inter)
                ...
                for inter1 in keys_actor:
                    data_other_actor_all[inter1] = torch.cat(
                        [data_other_actor_all[inter1], data_actor[inter1]], 0)

                for inter1 in keys_critic:
                    data_other_critic_all[inter1] = torch.cat(
                        [data_other_critic_all[inter1], data_critic[inter1]], 0)
            data_actor_all = data_other_actor_all
            data_critic_all = data_other_critic_all
            data_size = data_actor_all['state'].shape[0]
            for iter in range(self.train_actor_iters):
                ...
                for index in BatchSampler(SubsetRandomSampler(range(data_size)), batch_size, True):
                    ...
                    for _ in range(thread):
                        ...
                        loss_actor, actor_info = self.compute_loss_actor(self.split_batch(index, data_actor_all),inter)
                        kl = actor_info['kl']
            ...
            for iter in range(self.train_critic_iters):
                ...
                for index in BatchSampler(SubsetRandomSampler(range(data_size)), batch_size, True):
                    for _ in range(thread):
                        loss_critic, critic_info = self.compute_loss_critic_other(
                            self.split_batch(index, data_other_critic_all), self.split_batch(index, data_other_actor_all),
                            inter)
                        loss_critic /= (batch_num * thread)
                        loss_critic.backward()
    ...
    def compute_loss_critic_other(self, data,data_actor,index):
        state, next_state, order, action, advantage, oldp, mask_order, mask_action, mask_agent, hidden_rnn_actor = data_actor[
                                                                                                                       'state'], \
                                                                                                                   data_actor[
                                                                                                                       'next_state'], \
                                                                                                                   data_actor[
                                                                                                                   ...
                                                                                                                   data_actor[
                                                                                                                       'state_rnn']
        # Policy loss
        probs, _ = self.actor_l[index].multi_mask_forward(state, order, mask_order, self.adj,
                                                          hidden_rnn_actor.unsqueeze(0))
        # newp = torch.gather(probs, 2, action[:,:,None]).squeeze(-1)
        newp = torch.gather(probs, -1, action[..., None]).squeeze(-1)
        ratio = newp / oldp
        ratio[~mask_action] = 0
        ...
        if self.use_value_clip:
            ...
            value_loss = (ratio*torch.max(value_loss_original, value_loss_clipped)).mean()
            # value_loss = (torch.max(value_loss_original, value_loss_clipped)).mean()
        else:
            error_original = ret - new_value
            if self.use_huberloss:
