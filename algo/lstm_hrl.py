...
class PPO:
    ...
    def __init__(self,
        ...
        ### 上层LSTM
        self.LSTM_buffer=deque(maxlen=20)
        self.his = history(input_size=size_lstm,obs_size=65).to(self.device)
        self.loss_function = nn.MSELoss()
        self.optimizer_his = torch.optim.Adam(self.his.parameters(), lr=0.001)
    ...
    def action_process(self, action):
        ...
        return action
    def action_hrl(self,id,his, device='cpu'):
        action_prob=self.hrl_actor(torch.LongTensor(id).to(device),torch.FloatTensor(his).to(device))
    ...
    def his_train(self,device):
        ex_batch = random.sample(self.LSTM_buffer, 5)
        old_trj_l=[]
        new_trj_l=[]
        for inter in ex_batch:
            old_trj_l.append(inter[0].to(device))
            new_trj_l.append(inter[1].to(device))

        for iter in range(len(old_trj_l)):
            his=old_trj_l[iter]
            new_trj=new_trj_l[iter]
            state_action=new_trj[:,:,:65]
            state_reward=new_trj[:,:,65:]
            his=his.unsqueeze(0)
            his=his.repeat(len(state_action),1,1,1)
            self.optimizer_his.zero_grad()
            y_pred = self.his(his,state_action)
            single_loss = self.loss_function(y_pred, state_reward)
            single_loss.backward()
            self.optimizer_his.step()
            aaaa = 1
    ...
    def ppo_hrl_train(self,device):
        ...
        if lenth < 1:
            ...
            state, action, action_prob, reward,his= [], [], [], [],[]
            for inter in ex_batch:
            ...
            value_loss = F.smooth_l1_loss(
                self.hrl_critic(torch.LongTensor(state).to(device),his), target_v)
            ...
            ### 更新actor
            advantage = (target_v - self.hrl_critic(torch.LongTensor(state).to(device),his)).detach()
            advantage = (advantage - torch.mean(advantage)) / torch.std(advantage)
            # 更新actor
            new_prob = self.hrl_actor(torch.LongTensor(state).to(device),his)
            ...
            actor_loss = -torch.min(L1, L2).mean()- self.en_para * en_prefer.mean()
...
if __name__ == "__main__":
    ...
    '''
    trj=torch.zeros((288,grid_num,81))
    obs=torch.zeros((288,grid_num,65))
    his=history(input_size=15*2+50+1,output_size=15*2+50+1)
    aaa=his(trj,obs)
    aaaa=1